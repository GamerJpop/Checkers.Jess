...
function renderChecker(i, color) {
    ...
    return `
    <div id="checker-${i}" class="checker king-black-checker" bacon="${i}">
    </div>
    `    
    } 
    
    else if(checkers[i].isWhiteKing) {
    return `
    <div id="checker-${i}" class="checker king-gray-checker" bacon="${i}">
    </div>
    `    
    }
...
function selectChecker() {
    ...
    if(checker.hasClass(`selected`)) {
        console.log(`this checker is allready selected`)
        remove()
        return       
    }
